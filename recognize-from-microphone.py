...
if __name__ == '__main__':
  ...
  chunksize = 2**12  # 4096 tamanio de guardado
  ...
  reader = MicrophoneReader(None) #Inicializa los valores del micro por defecto
  ...
  while True:
    ...
    #Guarda todos los datos en el proceso de grabacion
  ...
  #Funcion que agrupa las muestras de la grabacion
  ...
  print colored(msg, attrs=['dark']) % len(data[0]) #Imprime el numero de muestras tomadas


  Fs = fingerprint.DEFAULT_FS # muestras por segundo
  channel_amount = len(data)

  result = set()
  matches = []

  #Busca canciones parecidas a la grabada

  def find_matches(samples, Fs=fingerprint.DEFAULT_FS):
    hashes = fingerprint.fingerprint(samples, Fs=Fs)
    return return_matches(hashes)

  # Retorna las canciones parecidas a la grabada

  def return_matches(hashes):
    mapper = {}
    #Recorre los hash de la cancion grabada y guarda las llaves de las canciones
